...
下記は ISDBScanner v1.1.5 時点でのインストール手順です。

```bash
# x86_64 環境
sudo wget https://github.com/tsukumijima/ISDBScanner/releases/download/v1.1.5/isdb-scanner -O /usr/local/bin/isdb-scanner
sudo chmod +x /usr/local/bin/isdb-scanner

# arm64 環境
sudo wget https://github.com/tsukumijima/ISDBScanner/releases/download/v1.1.5/isdb-scanner-arm -O /usr/local/bin/isdb-scanner
sudo chmod +x /usr/local/bin/isdb-scanner
```
